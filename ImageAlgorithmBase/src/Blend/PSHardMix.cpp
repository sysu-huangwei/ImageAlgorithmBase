...
namespace iab {
    ...
    unsigned char* PSHardMix::Run(unsigned char* imageA, unsigned char* imageB, int width, int height, int channel) {
        ...
        if (imageA != nullptr  &&  imageB != nullptr  &&  width > 0  &&  height > 0) {
            ...
            for (int i = 0; i < pixelsCount; i++) {
                if (channel == 4  ||  channel == 3) {
                    pResult[PS_RED] = pA[PS_RED] + pB[PS_RED] < 255.0 ? 0 : 255.0;
                    pResult[PS_GREEN] = pA[PS_GREEN] + pB[PS_GREEN] < 255.0 ? 0 : 255.0;
                    pResult[PS_BLUE] = pA[PS_BLUE] + pB[PS_BLUE] < 255.0 ? 0 : 255.0;
