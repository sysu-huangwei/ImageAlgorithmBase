...
namespace iab {
    ...
    unsigned char* PSScreen::Run(unsigned char* imageA, unsigned char* imageB, int width, int height, int channel) {
        ...
        if (imageA != nullptr  &&  imageB != nullptr  &&  width > 0  &&  height > 0) {
            ...
            for (int i = 0; i < pixelsCount; i++) {
                if (channel == 4) {
                    pResult[PS_RED] = 255.0 - (255.0 - pA[PS_RED]) * (255.0 - pB[PS_RED]) / 255.0;
                    pResult[PS_GREEN] = 255.0 - (255.0 - pA[PS_GREEN]) * (255.0 - pB[PS_GREEN]) / 255.0;
                    pResult[PS_BLUE] = 255.0 - (255.0 - pA[PS_BLUE]) * (255.0 - pB[PS_BLUE]) / 255.0;
                    pResult[PS_ALPHA] = 255.0 - (255.0 - pA[PS_ALPHA]) * (255.0 - pB[PS_ALPHA]) / 255.0;
                }
                else if (channel == 3) {
