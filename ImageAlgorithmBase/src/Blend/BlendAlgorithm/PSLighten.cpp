...
namespace iab {
    ...
    unsigned char* PSLighten::Run(unsigned char* imageA, unsigned char* imageB, int width, int height, int channel) {
        ...
        if (imageA != nullptr  &&  imageB != nullptr  &&  width > 0  &&  height > 0) {
            ...
            for (int i = 0; i < pixelsCount; i++) {
                if (channel == 4  ||  channel == 3) {
                    bool isAlighten = pA[PS_RED] + pA[PS_GREEN] + pA[PS_BLUE] > pB[PS_RED] + pB[PS_GREEN] + pB[PS_BLUE];
                    pResult[PS_RED] = isAlighten ? pA[PS_RED] : pB[PS_RED];
                    pResult[PS_GREEN] = isAlighten ? pA[PS_GREEN] : pB[PS_GREEN];
                    pResult[PS_BLUE] = isAlighten ? pA[PS_BLUE] : pB[PS_BLUE];
                    if (channel == 4) {
